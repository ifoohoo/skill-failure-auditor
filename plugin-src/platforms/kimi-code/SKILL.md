...
Kimi Code 当前只保留旧版直接编排兼容性，尚未声明 Loop 外层合同可执行。入口必须显式选择旧版兼容流程；缺少该选择或 Loop 合同不匹配时直接停止，禁止自动回退。

`$SKILL_ROOT/scripts/loop_audit_contract.py` 可以编译和校验平台无关的 SFA 领域数据，但当前 Kimi Code 入口不得据此宣称已经具备 Loop 承载能力。
