...
先从材料中读取实际记录的模型标识、版本或修订、配置、任务、时间和环境。材料没有记录的项目标为未知，不能根据实施者、审阅者或当前宿主补写。一个模型或配置的观察不能证明相邻模型、其他版本、不同环境或后续运行具有相同表现。

`runtime` 表示审阅已经产生的运行材料，不表示 SFA 运行了目标。`combined` 也只把静态定义与已有运行材料放在一起审阅。交付说明应引用材料中真实存在的范围事实；报告的通用限制不能代替这些具体引用。
