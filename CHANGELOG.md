...
- 将同一入口分成帮助、使用前提检查、接入分析和可靠性失效诊断四种职能。用户显式指定审计模式后，材料不足时只说明缺项与继续条件。
- 报告补充输入范围、运行证据和模型版本边界，并把后续验证或整改动作保留为建议，不自动执行目标、探针或修复。
- 统一通过 `ifoohoo/skill-family-hub` 提供插件市场来源。本插件仓继续发布四平台清单，不再携带独立的 `marketplace.json` 索引。
